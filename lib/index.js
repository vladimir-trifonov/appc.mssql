	...
	return Connector.extend({
		...
		create: function (Model, values, callback) {
			...
			request.query(query, function createQueryCallback(err, results) {
				...
				if (row) {
					var instance = Model.instance(this.transformRow(Model, row), true);
		...
		findAll: function (Model, callback) {
			...
			request.query(query, function findAllQueryCallback(err, results) {
				...
				for (var i = 0; i < results.length; i++) {
					var row = results[i],
						instance = Model.instance(this.transformRow(Model, row), true);
					if (primaryKeyColumn) { instance.setPrimaryKey(row[primaryKeyColumn]); }
					rows.push(instance);
				}
		...
		save: function (Model, instance, callback) {
			var table = this.getTableName(Model),
				...
				placeholders = _.without(_.keys(payload), 'id', 'timestamp').map(function (key) { return key + ' = @' + key; }),
		...
		addValuesToSQLRequest: function addValuesToSQLRequest(Model, values, request, excludeTimestamp) {
			var tableSchema = this.getTableSchema(Model);
			for (var key in values) {
				if (values.hasOwnProperty(key)) {
					var schema = tableSchema[key],
						type = sql.VarChar(255),
						stringType = schema.DATA_TYPE.toLowerCase();
		...
		transformRow: function (Model, row) {
			for (var key in row) {
