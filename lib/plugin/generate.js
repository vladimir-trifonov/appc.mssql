...
function generate(opts, callback) {
	...
	async.series([
		...
		function connectConnector(cb) {
			tempConnector = new (require('../index').create(apibuilder))({
				mssql: localConfig
			});
		...
		function writeModels(cb) {
			async.eachSeries(tables, function forEachTable(table, done) {
				...
				for (var key in schema.objects[table]) {
					if (schema.objects[table].hasOwnProperty(key)) {
				...
				buffer += "var Model = APIBuilder.createModel('" + table + "'," + JSON.stringify(obj, '\t', 4) + ");\n\n";
