...
exports.createModelsFromSchema = function () {
	var self = this,
		models = {},
		exposePrimaryKeyAsId = _.has(Arrow.getGlobal(), 'config.serialization.exposePrimaryKeyAsId') ? _.get(Arrow.getGlobal(), 'config.serialization.exposePrimaryKeyAsId') : false;
	...
	function getFieldsExceptPrimaryKey(modelName, modelSchema) {
		return Object.keys(modelSchema)
			.filter(function (fieldName) {
				...
				return exposePrimaryKeyAsId && typeof primaryKeyColumnName !== 'undefined' && fieldName === primaryKeyColumnName ? false : (fieldName === 'id' ? false : true);
			})
			.reduce(function (fields, fieldName) {
				fields[fieldName] = {
					...
					readonly: modelSchema.DATA_TYPE === 'timestamp',
					custom: modelSchema.DATA_TYPE === 'timestamp'
				};
	...
	Object.keys(self.schema.objects)
		.filter(function (modelName) {
			var hasPk = typeof getPrimaryKeyFromSchema(modelName) === 'undefined';
			if (!hasPk) {
		...
		.forEach(function (modelName) {
			...
			models[self.name + '/' + modelName] = genModel(modelName, fields);
		});
